...
int i2c_init()
{
	...
	HAL_Delay(I2C_DELAY);
	while (HAL_GPIO_ReadPin(GPIOB, I2C1_SDA_GPIO_PIN) != GPIO_PIN_SET
			|| HAL_GPIO_ReadPin(GPIOB, I2C1_SCL_GPIO_PIN) != GPIO_PIN_SET)
		;
	HAL_GPIO_WritePin(GPIOB, I2C1_SDA_GPIO_PIN, GPIO_PIN_RESET);
	HAL_GPIO_WritePin(GPIOB, I2C1_SCL_GPIO_PIN, GPIO_PIN_RESET);
	HAL_Delay(I2C_DELAY);
	while (HAL_GPIO_ReadPin(GPIOB, I2C1_SDA_GPIO_PIN) != GPIO_PIN_RESET
			|| HAL_GPIO_ReadPin(GPIOB, I2C1_SCL_GPIO_PIN) != GPIO_PIN_RESET)
		;
	HAL_GPIO_WritePin(GPIOB, I2C1_SDA_GPIO_PIN, GPIO_PIN_SET);
	HAL_GPIO_WritePin(GPIOB, I2C1_SCL_GPIO_PIN, GPIO_PIN_SET);
	HAL_Delay(I2C_DELAY);
...
int i2c_read(uint8_t address, uint8_t * dest, size_t length)
{
	...
	do
	{
		ret = HAL_I2C_Master_Receive(&i2c_handle, address, dest, length,
				HAL_MAX_DELAY);
	} while (ret != HAL_OK);
	i2c_de_init();
	return ret;
...
int i2c_write(uint8_t address, uint8_t * src, size_t length)
{
	...
	do
	{
		ret = HAL_I2C_Master_Transmit(&i2c_handle, address, src, length,
			HAL_MAX_DELAY);
	} while (ret != HAL_OK);
	i2c_de_init();
	return ret;
		...
		size_t length)
{
	...
	do
	{
		ret = HAL_I2C_Mem_Read(&i2c_handle, address, mem_address,
		...
		size_t length)
{
	...
	do
	{
		ret = HAL_I2C_Mem_Write(&i2c_handle, address, mem_address,
